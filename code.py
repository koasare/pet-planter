...
label_temp = Label(font_small, max_glyphs=4)
label_temp.x = 35
label_temp.y = 300
splash.append(label_temp)

# Create a label to display the water level
label_level = Label(font_small, max_glyphs=4)
...
# initial number of alarms
alarm_sounded_max = False
alarm_sounded_min = False
...
while True:
    ...
    if moisture <= SOIL_LEVEL_MIN and alarm_sounded_min == False:
        ...
        alarm_sounded_min = True
    elif moisture >= SOIL_LEVEL_MAX and alarm_sounded_max == False:
        print("Playing high water level warning...")
        pyportal.play_file(wav_water_high)
        alarm_sounded_max = True
